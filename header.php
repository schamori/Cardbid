...
<nav class="nav-desktop">
  ...
  <div class="nav-desktop-logo-wrapper">
    <a href="<?php echo esc_url(home_url('/')); ?>" class="nav-desktop-logo-link">
      <div class="nav-desktop-logo">
        <div class="nav-desktop-logo-icon">
          <img src="https://cardbid.eu/wp-content/uploads/2025/10/Cardbid_Maskottchen_White.png" alt="Cardbid Logo" />
        </div>
      <div class="nav-desktop-logo-text">
        <span>Cardbid</span>
      </div>
    </div>
    </a>
  ...
  <div class="nav-desktop-cart-wrapper">
      ...
      <svg class="nav-desktop-account-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" data-account-url="<?php echo esc_url( wc_get_page_permalink( 'myaccount' ) ); ?>">
      ...
      <a href="<?php echo esc_url( wc_get_page_permalink( 'cart' ) ); ?>" class="nav-desktop-cart" title="Shopping Cart">
        <?php if ( function_exists( 'WC' ) ) : ?>
          ...
          <span class="nav-desktop-cart-price"><?php echo WC()->cart->get_cart_total(); ?></span>
        <?php else : ?>
          <span class="nav-desktop-cart-items">0 Items</span>
          <span class="nav-desktop-cart-price">0,00€</span>
        <?php endif; ?>
      </a>
      <a href="<?php echo esc_url( wc_get_page_permalink( 'cart' ) ); ?>" class="nav-desktop-cart-icon-container" title="Shopping Cart">
        <svg class="nav-desktop-cart-icon-svg" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <?php if ( function_exists( 'WC' ) && WC()->cart->get_cart_contents_count() > 0 ) : ?>
          <span class="nav-desktop-cart-count-badge"><?php echo WC()->cart->get_cart_contents_count(); ?></span>
        <?php endif; ?>
      </a>

      <!-- Cart Dropdown -->
      <div class="nav-desktop-cart-dropdown">
        <?php if ( function_exists( 'WC' ) && WC()->cart->get_cart_contents_count() > 0 ) : ?>
          <div class="nav-desktop-cart-dropdown-items">
            <?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
              $_product = apply_filters( 'woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key );
              if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 ) :
                $product_permalink = $_product->is_visible() ? $_product->get_permalink( $cart_item ) : '';
            ?>
              <div class="nav-desktop-cart-dropdown-item">
                <?php if ( $product_permalink ) : ?>
                  <a href="<?php echo esc_url( $product_permalink ); ?>" class="nav-desktop-cart-item-image">
                <?php else : ?>
                  <div class="nav-desktop-cart-item-image">
                <?php endif; ?>
                  <?php echo $_product->get_image( 'thumbnail' ); ?>
                <?php echo $product_permalink ? '</a>' : '</div>'; ?>

                <div class="nav-desktop-cart-item-details">
                  <?php if ( $product_permalink ) : ?>
                    <a href="<?php echo esc_url( $product_permalink ); ?>" class="nav-desktop-cart-item-name">
                      <?php echo wp_kses_post( $_product->get_name() ); ?>
                    </a>
                  <?php else : ?>
                    <span class="nav-desktop-cart-item-name"><?php echo wp_kses_post( $_product->get_name() ); ?></span>
                  <?php endif; ?>
                  <div class="nav-desktop-cart-item-meta">
                    <span class="nav-desktop-cart-item-quantity">Qty: <?php echo $cart_item['quantity']; ?></span>
                    <span class="nav-desktop-cart-item-price">
          ...
          <div class="nav-desktop-cart-dropdown-footer">
            <div class="nav-desktop-cart-dropdown-total">
              <span>Subtotal:</span>
              <strong><?php echo WC()->cart->get_cart_subtotal(); ?></strong>
            </div>
            <div class="nav-desktop-cart-dropdown-actions">
              <a href="<?php echo esc_url( wc_get_page_permalink( 'cart' ) ); ?>" class="nav-desktop-btn-view-cart">View Cart</a>
              <a href="<?php echo esc_url( wc_get_page_permalink( 'checkout' ) ); ?>" class="nav-desktop-btn-checkout">Checkout</a>
            </div>
          </div>
        <?php else : ?>
          <div class="nav-desktop-cart-dropdown-empty">
  ...
  <div class="nav-desktop-content-wrapper">
    ...
    <div class="nav-desktop-row-top">
      <!-- FiboSearch Integration -->
      <div class="nav-desktop-search">
        <?php
    ...
    <div class="nav-desktop-row-bottom">
      <div class="nav-desktop-links">
        ...
        foreach ($game_categories as $game) {
          ...
          if ($game['slug'] === 'all-games') {
            echo '<a href="' . esc_url(home_url('/shop/')) . '" class="nav-desktop-link">' . esc_html($game['name']) . '</a>';
            continue;
          }

          $category = get_term_by('slug', $game['slug'], 'product_cat');
          if ($category) {
            echo '<a href="' . esc_url(get_term_link($category)) . '" class="nav-desktop-link nav-desktop-game-link" data-game="' . esc_attr($game['slug']) . '">';
  ...
  <div class="nav-desktop-mega-menu">
    <div class="nav-desktop-mega-menu-content">
      <?php
      ...
      foreach ($game_categories as $game) {
        ...
        echo '<div class="nav-desktop-mega-menu-game" data-game="' . esc_attr($game['slug']) . '">';
        echo '<ul class="nav-desktop-mega-menu-list">';
        ...
        if (!empty($expansions)) {
          foreach ($expansions as $expansion) {
            ...
            echo '<li class="nav-desktop-expansion-item">';
            echo '<span class="nav-desktop-expansion-name">' . esc_html($expansion->name) . '</span>';
            echo '<span class="nav-desktop-expansion-types">';
            echo '<a href="' . esc_url($singles_url) . '" class="nav-desktop-type-link">S</a>';
            echo '<a href="' . esc_url($sealed_url) . '" class="nav-desktop-type-link">B</a>';
            echo '</span>';
            echo '</li>';
          }
        }
...
<nav class="nav-mobile">
  ...
  <div class="nav-mobile-logo-wrapper">
    <a href="<?php echo esc_url(home_url('/')); ?>" class="nav-mobile-logo-link">
      <div class="nav-mobile-logo">
        <div class="nav-mobile-logo-icon">
          <img src="https://cardbid.eu/wp-content/uploads/2025/10/Cardbid_Maskottchen_White.png" alt="Cardbid Logo" />
        </div>
        <div class="nav-mobile-logo-text">
          <span>Cardbid</span>
        </div>
      </div>
    </a>
  ...
  <!-- Mobile icons group -->
  <div class="nav-mobile-icons-group">
    <!-- Account Icon -->
    <div class="nav-mobile-account-wrapper">
      <a href="<?php echo esc_url( wc_get_page_permalink( 'myaccount' ) ); ?>" class="nav-mobile-account-icon" title="My Account">
    ...
    <!-- Cart -->
    <div class="nav-mobile-cart-wrapper">
      <a href="<?php echo esc_url( wc_get_page_permalink( 'cart' ) ); ?>" class="nav-mobile-cart" title="Shopping Cart">
        <div class="nav-mobile-cart-icon-container">
          <svg class="nav-mobile-cart-icon-svg" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M6.21178 1.76158C6.3478 1.4046 6.55839 1.07661 6.83374 0.801273C7.34678 0.288227 8.04262 0 8.76818 0H12.6682C13.3937 0 14.0896 0.288227 14.6026 0.801273C14.8694 1.06809 15.0755 1.38434 15.2117 1.7285C16.1135 1.99696 16.9319 2.50111 17.5796 3.19386C18.3702 4.0396 18.8635 5.12021 18.9843 6.27164L19.2389 8.58665C20.558 8.68718 21.5621 9.90051 21.3877 11.2307L20.2987 19.4356C20.2952 19.4611 20.2917 19.4909 20.2878 19.5245C20.2641 19.7287 20.2243 20.0697 20.0961 20.3788C19.82 21.0444 19.2389 21.5613 18.55 21.7615C18.2225 21.8565 17.8893 21.8537 17.6854 21.8518L17.6164 21.8515H3.89317C3.86554 21.8515 3.83324 21.852 3.79698 21.8526C3.57442 21.8564 3.20334 21.8625 2.85523 21.7615C2.16625 21.5613 1.58515 21.0444 1.30905 20.3788C1.16211 20.0244 1.1184 19.6028 1.09206 19.3487C1.08645 19.2945 1.08163 19.248 1.07676 19.2114L0.0383467 11.3877C0.0142331 11.206 -0.0219679 10.931 0.0175614 10.6306C0.14669 9.6492 0.896788 8.83126 1.87503 8.62806C1.98035 8.60618 2.08808 8.59151 2.19764 8.58458L2.45242 6.26853L2.45263 6.26658C2.5806 5.12066 3.07747 4.04733 3.86837 3.20828C4.51151 2.526 5.32078 2.02866 6.21178 1.76158ZM6.45075 4.18959C6.12741 4.34578 5.83307 4.56045 5.5836 4.82511C5.14337 5.29214 4.86673 5.8895 4.79531 6.52731L4.56951 8.58002H16.8668L16.641 6.52629L16.6402 6.51961C16.5733 5.87767 16.2986 5.27513 15.8578 4.80367C15.6109 4.53966 15.3195 4.32484 14.9993 4.16761C14.8874 4.34966 14.7544 4.5184 14.6026 4.67017C14.3486 4.9242 14.047 5.12571 13.7151 5.26319C13.3832 5.40067 13.0274 5.47144 12.6682 5.47144H8.76818C8.04262 5.47144 7.34678 5.18321 6.83374 4.67017C6.68726 4.52369 6.5591 4.36231 6.45075 4.18959ZM6.57164 13.2657C6.57164 12.7233 6.13192 12.2836 5.5895 12.2836C5.04709 12.2836 4.60735 12.7233 4.60735 13.2657V17.1657C4.60735 17.7081 5.04709 18.1478 5.5895 18.1478C6.13192 18.1478 6.57164 17.7081 6.57164 17.1657V13.2657ZM10.7182 12.2836C11.2606 12.2836 11.7003 12.7233 11.7003 13.2657V17.1657C11.7003 17.7081 11.2606 18.1478 10.7182 18.1478C10.1757 18.1478 9.73602 17.7081 9.73602 17.1657V13.2657C9.73602 12.7233 10.1757 12.2836 10.7182 12.2836ZM16.8289 13.2657C16.8289 12.7233 16.3893 12.2836 15.8468 12.2836C15.3044 12.2836 14.8647 12.7233 14.8647 13.2657V17.1657C14.8647 17.7081 15.3044 18.1478 15.8468 18.1478C16.3893 18.1478 16.8289 17.7081 16.8289 17.1657V13.2657Z" fill="white"/>
          </svg>
          <?php if ( function_exists( 'WC' ) && WC()->cart->get_cart_contents_count() > 0 ) : ?>
            <span class="nav-mobile-cart-count-badge"><?php echo WC()->cart->get_cart_contents_count(); ?></span>
          <?php endif; ?>
        </div>
      </a>
    </div>
    ...
    <!-- Mobile Menu Toggle -->
    <button class="nav-mobile-menu-toggle" id="mobileMenuToggle" aria-label="Toggle mobile menu">
      <span></span>
      <span></span>
      <span></span>
    </button>
